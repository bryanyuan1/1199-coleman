...
- **Dev loop** (teal, Claude Code): plan.md → dev-plan-review → human verify → dev-plan-critique → human verify → dev-plan-edit → human approve → dev-plan-execute. An extra "approve" gate before any code execution.
...
### Composite (15) — Orchestration workflows

| Skill | Trigger phrases | What it does |
|-------|----------------|-------------|
| `knowledge-retrieval` | "how does X work", "what is Y" | Fast-path lookup: wiki + PRDs + local code |
| `progress-digest` | "current status", "generate email" | Cross-reference tracker + PRDs + logs → report |
| `paper-distill` | "distill this paper", "add to wiki" | Paper → six-section wiki entry + cross-ref propagation |
| `experiment-design` | "design an experiment", "test plan" | Step-by-step spec card with every command explicit |
| `prd-draft` | "write a PRD", "draft design doc" | Chat discussion → first PRD version with gap comments |
| `prd-critique` | "critique this PRD", "find problems" | AI finds problems → posts inline comments |
| `prd-edit` | "update the PRD", "incorporate feedback" | Resolve comments + edit content + post new questions |
...
**Dev plan lifecycle:**
```
plan.md → dev-plan-review → dev-plan-critique → dev-plan-edit → dev-plan-execute
```
